...
class DecoderOnlyTransformer(nn.Module):
    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        if target_lengths is not None:
            ...
            #PadMask(target_lengths, padded_targets.size(1)).to(padded_targets.device)
        ...
        #CausalMask(padded_targets.size(1)).to(padded_targets.device)
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def decode(
        ...
        pad_mask_tgt = None#PadMask(x_enc, x_enc_lengths).to(padded_targets.device)
        if target_lengths is not None:
        ...
        causal_mask = CausalMask(padded_targets).to(padded_targets.device)
        #print(f"Causal mask for seq_len {padded_targets.size(1)}:\n{causal_mask}")
