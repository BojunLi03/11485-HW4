...
class LMDataset(Dataset):
    ...
    def __init__(
       ...
       # raise NotImplementedError # Remove once implemented
        ...
        for file in tqdm(self.text_files):
            ...
            file = os.path.join(self.text_dir, file)
    ...
    def collate_fn(self, batch: List[Tuple[torch.LongTensor, torch.LongTensor]]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        padded_shifted = pad_sequence(shifted_transcripts, True, self.pad_token) # (B, T)
        padded_golden  = pad_sequence(golden_transcripts, True, self.pad_token) # (B, T)
